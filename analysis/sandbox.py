...
## ----------squeeze
with pd.HDFStore('/Users/bdd/data/fus_anes/2025-07-22_13-56-10_subject-b001.h5', 'r') as h:
    eeg = h.eeg
    bl_eyes = h.bl_eyes
    markers = h.markers
    sq = h.squeeze
...
with pd.HDFStore('/Users/bdd/data/fus_anes/2025-07-22_13-56-10_subject-b001.h5', 'r') as h:
    ...
    chirp = h.chirp
eeg_time = eeg.index.values
data = eeg.iloc[:,:15].values.T * 1e-6
sfreq = 500.0  # Hz
from fus_anes.constants import MONTAGE
channel_names = MONTAGE[:15]
def t2i(t):
...
power.plot(picks='FCz', baseline=(-0.1, 0), mode='logratio',
           title='Evoked power')

# Inter-trial coherence (phase-locking)
itc.plot(picks='FCz', title='ITC')
...
onset_idx -= int(0.004 * fs)# this is a manual pullback because i have to detect peaks but the pulse started before those peaks, visually verify its right
...
#cols = ['slateblue', 'grey', 'red', 'pink', 'green', 'gold',]# 'k']
#cnames = ['P7', 'P8', 'F3', 'M1', 'C3', 'P7',]# 'ssep']
cols = ['slateblue', 'grey']
cnames = ['C3', 'C4']
